"""
Generic Command Injection Scanner
"""
...
class CommandInjectionScanner(BaseScanner):
    ...
    def test_url(self, url, method='GET'):
        self.logger.debug(f"Testing URL for command injection: {url}")
        parsed = urlparse(url)
        ...
        for param in params:
            for payload in self.payloads:
                ...
                try:
                    ...
                    if self._is_command_injection_successful(response.text):
                        self.logger.warning(f"💉 Command injection found: {url} param={param} payload={payload}")
    ...
    def test_form(self, form):
        self.logger.debug(f"Testing form for command injection: {form['url']}")
        ...
        for inp in testable:
            ...
            for payload in self.payloads:
                ...
                try:
                    ...
                    if self._is_command_injection_successful(response.text):
                        self.logger.warning(f"💉 Command injection found in form {form['url']} param={param} payload={payload}")
                        self.vulnerabilities_found.append({
                            "type": "Command Injection",
                            "url": form['url'],
                            "parameter": param,
    ...
    def _is_command_injection_successful(self, response_text):
        ...
        for indicator in self.injection_indicators:
            if indicator.lower() in text:
                return True
        # Also check for ping output
        if "pinging" in text and "bytes from" in text:
            return True
        return False